...
public abstract class AbstractServer implements Runnable {
	...
	public void sendToClients(Object msg, String userId) {
		Thread[] clientThreadList = getClientConnections();

		for (int i = 0; i < clientThreadList.length; i++) {
			if (((ConnectionToClient) clientThreadList[i]).getUserId().equals(userId)) {
				try {
					((ConnectionToClient) clientThreadList[i]).sendToClient(msg);
				} catch (IOException e) {
					e.printStackTrace();
	 ...
	 * 
	 * The default implementation does nothing. The method may be overridden by
